...
entity_prototype entity_prototype_get(entity_type et) {
    if (!inited) {
        entity_prototypes[ET_PLAYER] = (entity_prototype){"player", "pP", {64, 0, 16, 16}, true};
        entity_prototypes[ET_CRATE] = (entity_prototype){"crate", "cC", {96, 0, 16, 16}, true};
        entity_prototypes[ET_PRESENT] = (entity_prototype){"present", "bB", {48, 0, 16, 16}, true};
        entity_prototypes[ET_TARGET] = (entity_prototype){"target", "tT", {80, 0, 16, 16}, false};
        entity_prototypes[ET_NONE] = (entity_prototype){"none", "", {0, 0, 0, 0}, false};
    }
    inited = true;
